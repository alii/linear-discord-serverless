 ...
 */
export function getPriorityValue(priority: NonNullable<Issue["priority"]>): StringPriority {
	switch (priority) {
		case 0:
			return "None";
		case 1:
...
export function parseImages(content: string): {
	...
	return {
		images: content.match(/\b(https?:\/\/\S+(?:png|jpe?g|gif|webm)\S*)\b/g) || [],
...
export async function sendComment(
	...
	const type: "Update" | "Create" = metadata.action === "create" ? "Create" : "Update";
...
export async function sendIssue(
	...
	const embed = new MessageEmbed()
		...
		.setColor((payload?.state?.color ?? "#4752b2") as HexColorString)
